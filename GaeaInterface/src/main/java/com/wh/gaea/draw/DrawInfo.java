...
public abstract class DrawInfo{
			...
			if (sizeString.indexOf('%') != -1){
				...
				case stRight:
					if (parent == null || parent.isEmpty())
						...
						IUINode parentNode = (IUINode)node.getCanvas().getNode(parent);
						if (parentNode == null)
							parentSize = node.getCanvas().getPageSize().width;
						else
							parentSize = parentNode.getDrawInfo().getWidth();
					}
					break;
				case stTop:
				case stBottom:
					...
					if (parent == null || parent.isEmpty())
						...
						parentSize = parentNode.getDrawInfo().getHeight();
					}
					break;
				default:
					break;
		...
		if (node.getRefRect() == null){
			...
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					...
					nodes.add(node);
					((IDrawCanvas)node.getCanvas()).refreshDrawTree(nodes, false);
				}
			});
		}
	...
	public void drawNode(Graphics g, Rectangle rect){
		...
		if ((drawTypes & DrawType_Text) == DrawType_Text){
			if (value != null)
				IDrawNode.drawLineText(g, getFont(), textColor, rect.x, rect.y, rect.width, rect.height, value.toString());
		}
	}

	public DrawInfo(IUINode node){
		if (node == null)
			return;
