...
public class CodeFlowSelectDialog extends JDialog {
	...
	IDrawCanvas canvas = GlobalInstance.instance().createAppWorkflowCanvas();
	...
	public void init() throws Exception{
		File[] files = GlobalInstance.instance().getAppWorkflowFiles();
	...
	public void load(String name) {
		try {
			File f = GlobalInstance.instance().getProjectFile(IEditorEnvironment.AppWorkflow_Dir_Name, GlobalInstance.instance().getAppWorkflow_FileName(name));
			if (f == null)
				return;
...
//					MsgHelper.showMessage(this, "文件不存在！", "提示", JOptionPane.WARNING_MESSAGE);
			...
			canvas.load(null, new IInitPage() {
				...
				public void onPage(IDrawPageConfig pageConfig) {
					canvas.getPageConfig().setPageSizeMode(pageConfig.getCurPageSizeMode(), pageConfig.getWidth(), pageConfig.getHeight());
				}
			});
