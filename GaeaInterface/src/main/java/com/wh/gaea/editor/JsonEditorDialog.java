...
public class JsonEditorDialog extends JDialog {
	...
	protected void removeTreeNode() {
		...
		if (MsgHelper.showConfirmDialog("是否删除选定的节点？", "删除节点",
	...
	protected DefaultMutableTreeNode addTreeNode(DefaultTreeModel model, 
		...
		if (parent == model.getRoot()) {
			if (!(name.equals("{}") || name.equals("[]"))) {
				MsgHelper.showMessage(null, "不能在根节点添加此类节点！", "添加", JOptionPane.WARNING_MESSAGE);
				return null;
			}
		}
		...
		case "[]":
			...
			MsgHelper.showMessage(null, "此节点不可以添加节点！", "添加", JOptionPane.WARNING_MESSAGE);
			return null;
		default:
			if (parent.getUserObject() instanceof TreeItemInfo) {
				String parentName = parent.getUserObject().toString();
	...
	protected void updateTree() {
		...
		try {
			...
			e.printStackTrace();
			MsgHelper.showMessage(null, "数据格式错误，请检查输入的数据！", "错误", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	protected void initTemplates(String typename) {
		File controlTemplateDir = GlobalInstance.instance().getEditorSourcePath(IEditorEnvironment.Template_Path,
				IEditorEnvironment.Control_Dir_Name);
		...
		for (File file : controlTemplateDir.listFiles(new FileFilter() {
			...
			public boolean accept(File file) {
				return FileHelp.GetExt(file.getName()).compareTo(IEditorEnvironment.Template_Path) == 0;
			}
		})) {
	...
	public JsonEditorDialog(IMainControl mainControl) {
		...
		controlTypeComboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				gridColumnType.setVisible(item.name.compareTo(IDrawInfoDefines.Grid_Name) == 0);
			}
		});
		...
		for (String typename : IDrawInfoDefines.TypeNames) {
			ImageIcon icon = new ImageIcon(GlobalInstance.instance().getUIControlImage(typename));
		...
		menuItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				IUINode node = CreateControlDialog.showDialog(null, null, false);
				if (node != null) {
					try {
		...
		menuItem_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				ControlSelectDialog.Result result = ControlSelectDialog.showDialog(text);
				if (!result.isok)
					return;
		...
		menuItem_6.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ModelflowSelectDialog.Result result = ModelflowSelectDialog.showDialog(JsonEditorDialog.this.mainControl,
		...
		menuItem_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				File ImageFile = SwingTools.selectOpenImageFile(null, null, null);
				if (ImageFile == null)
					return;

				String value = ImageFile.getName();
				File dest = GlobalInstance.instance().getProjectFile(IEditorEnvironment.Image_Resource_Path, value);
				try {
