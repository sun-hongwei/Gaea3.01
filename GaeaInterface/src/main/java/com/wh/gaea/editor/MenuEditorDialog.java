...
import com.wh.gaea.editor.JsonTreeDataEditor.ChangeObjectType;
import com.wh.gaea.editor.JsonTreeDataEditor.ChangeType;
import com.wh.gaea.editor.JsonTreeDataEditor.IChange;
import com.wh.gaea.editor.JsonTreeDataEditor.TreeItemInfo;
...
public class MenuEditorDialog extends JDialog {
	...
	protected void onClose() {
		if (isEdit) {
			int ret = MsgHelper.showConfirmDialog("数据已经修改，是否退出", "退出", JOptionPane.YES_NO_CANCEL_OPTION);
			switch (ret) {
	...
	public MenuEditorDialog(IMainControl mainControl) {
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					e1.printStackTrace();
					MsgHelper.showMessage(null, "刷新失败！", "刷新", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
		panel.add(button);
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (jsonTreeDataEditor.tree.getSelectionPath() == null
						|| jsonTreeDataEditor.tree.getSelectionPath().getLastPathComponent() == null) {
					MsgHelper.showMessage(null, "请先选择一个节点", "提示", JOptionPane.INFORMATION_MESSAGE);
					return;
				}

				DefaultMutableTreeNode node = (DefaultMutableTreeNode) jsonTreeDataEditor.tree.getSelectionPath()
						.getLastPathComponent();

				if (!node.isLeaf()) {
					MsgHelper.showMessage(null, "只有无子节点的节点才可以关联！", "提示", JOptionPane.INFORMATION_MESSAGE);
					return;
				}
		...
		button_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!node.isLeaf()) {
					MsgHelper.showMessage(null, "只有叶子节点才可以关联！", "提示", JOptionPane.INFORMATION_MESSAGE);
					return;
				}
	...
	public static File getMainMenuFile() {
		return GlobalInstance.instance().getProjectFile(IEditorEnvironment.Menu_Dir_Path,
	...
	public void refresh() {
		...
		if (file.exists()) {
			JSONArray obj;
			try {
				obj = (JSONArray) JsonHelp.parseCacheJson(file, null);
	...
	protected Component addMenu(JSONObject data, JMenuItem parentNode, TreeMap<String, List<JSONObject>> all)
	...
	protected void jsonToMenu(String pid, List<JSONObject> datas, JMenuItem parentNode,
			TreeMap<String, List<JSONObject>> all) throws JSONException {
	...
	public void save() throws Exception {
		...
		isEdit = false;
		GlobalInstance.instance().lockFile(getFile());
	}
