...
$command = isset ( $require ["command"] ) ? $require ["command"] : NULL;
...
if ($command != "getdate" && !isset($_POST["global"]))
{
...
else if ($command == "getDebuggerInfo")
{
	$uri = array();
	$uri["ret"] = true;
	$uri["debugger"] = Configure::get("DebuggerModel", "debugger");
	$str = json_encode($uri);
	echo $str;
}
?>