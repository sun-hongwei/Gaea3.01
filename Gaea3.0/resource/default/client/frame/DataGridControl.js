...
function GLOBAL_DATAGRID_CELLCLICK(gridid, linkString, field, rowIndex, value,row) {
    var grid = mini.get(gridid);
    var dgc = grid.options[GridOptionsKey];

    var link = JSON.parse(linkString.replace(/'/g, "\""));
    var row = JSON.parse(row.replace(/'/g, "\""));
    var dotype = "click";
    ...
    switch (dotype) {
        case "click":
            dgc.doCellClick(link, field, rowIndex, value,row);
            break;
        case "uiid":
            dgc.doCellJump(link, field, rowIndex, value,row);
            break;
    }
}
...
function DataGridControl() {
    ...
    this.doCellJump = function (link, field, rowIndex, value,row) {
        var name = link.uiname;
        jumpUIForName(name, link.uiid,undefined,row);
    ...
    this.setLink = function (e, gid, link, name) {
        ...
        if (link.hasOwnProperty("url")) {
            ...
            e.cellHtml = '<a href="javascript:GLOBAL_DATAGRID_CELLCLICK(\'' + gid + '\'' +
                ...
                ', \'' + e.rowIndex + '\'' +
                ', \'' + e.value + '\'' +
                ', \'' + JSON.stringify(e.row).replace(/\"/g, "\\'") + '\'' +
                ')" ' +
        ...
        if (Tools.isNull(header.field)) {
            ...
            GlobalDataSources.get(dataSourceId, true,
                function (ds, columns, dataset, url, queryParams, params) {
                    ...
                    dg.directLoadData(dataset, params["start"], params["size"], ds.remote_params);
