function ControlFactory() {
    ...
    this.write = function (dw, data, width, height, newTabProc, onParentResize) {
        ...
        for (i = 0; i < data.data.length; i++) {
            ...
            if (Tools.isNull(item.data.popup))
                continue;
            while (true) {
                var popupDiv = getFrameControlById(data, item.data.popup);
                if (!Tools.isNull(popupDiv)) {
                    $(popupDiv).parent().remove();
                } else
                    break;
            }
        }
