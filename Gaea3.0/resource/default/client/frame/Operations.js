...
function traversePopupDiv(eachProc) {
    var divObj = $(document.body).children(".mini-popup");
    if (Tools.isNull(divObj) || divObj.length == 0)
        return;

    for(var i = 0; i < divObj.length; i++){
        var findObj = $(divObj[i]).children("div");

        if (Tools.isNull(findObj) || findObj.length == 0)
            continue;

        findObj = findObj[0];
        if (!Tools.isNull(findObj)) {
            var isok = eachProc(-1, findObj);
