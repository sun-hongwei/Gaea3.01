...
import com.wh.gaea.editor.ControlSelectDialog.Result;
...
import com.wh.swing.tools.tree.ImageTreeRender;
...
public class SceneBuilder extends ChildForm implements IMainMenuOperation, ISubForm {
	...
	class TreeControl {
		...
		protected void renameTreeNode() {
			...
			String name = MsgHelper.showInputDialog("请输入节点的新名字：", node.getUserObject().toString());
		...
		protected void addTreeNode(NodeType nodeType) {
			...
			case ntDirNode:
				if (info != null && info.type() != NodeType.ntDirNode) {
					MsgHelper.showMessage(null, "仅可以在类型为【目录】的节点下添加【目录】！", "提示", JOptionPane.WARNING_MESSAGE);
					return;
				}
				break;
			case ntExceptNode:
				if (info.type() != NodeType.ntDirNode) {
					MsgHelper.showMessage(null, "仅可以在类型为【目录】的节点下添加【异常场景】！", "提示", JOptionPane.WARNING_MESSAGE);
					return;
				}
				break;
			case ntMainNode:
				if (info.type() != NodeType.ntDirNode) {
					for (int i = 0; i < selectNode.getChildCount(); i++) {
						...
						if (child.type() == NodeType.ntMainNode) {
							MsgHelper.showMessage(null, "一个【目录】下仅可以一个【主场景】！", "提示",
									JOptionPane.WARNING_MESSAGE);
							return;
						}
					}
					MsgHelper.showMessage(null, "仅可以在类型为【目录】的节点下添加【主场景】！", "提示", JOptionPane.WARNING_MESSAGE);
					return;
				}
			...
			if (name == null)
				name = MsgHelper.showInputDialog("请输入节点的新名字：", "新场景");
			if (name == null || name.isEmpty())
				return;
	...
	class TableControl {
		...
		protected void remove() {
			...
			if (MsgHelper.showConfirmDialog("是否删除选定的记录？", "删除",
	...
	public void load() {
		...
		Object object;
		try {
			object = JsonHelp.parseCacheJson(file, null);
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (casename == null || casename.isEmpty()) {
						MsgHelper.showMessage("业务流程图只能在场景节点上使用！");
						return;
					}
		...
		button_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (result.isok && result.data != null) {
					tableControl.add((DrawNode) result.data[0]);
				}
			}
		});
	...
	public static SceneInfo parseJson(File appDesignFile) throws Exception {
		...
		try {
			JSONArray valueObj = (JSONArray) JsonHelp.parseCacheJson(appDesignFile, null);
