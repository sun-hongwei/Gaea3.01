...
import com.wh.gaea.editor.WorkflowSelectDialog;
...
public class WorkflowBuilder extends ChildForm implements IMainMenuOperation, ISubForm {
	...
	public void addNode() {
		...
		case FlowNode.SubNode_Title: {
			String name;
			if (MsgHelper.showConfirmDialog("是否新建子流程？", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
	...
	protected void onEditNode(DrawNode node) {
		if (node instanceof ChildFlowNode) {
			...
			if (file.exists())
				...
				if (MsgHelper.showConfirmDialog("未发现子流程，是否新建？",
	...
	public WorkflowBuilder(IMainControl mainControl) throws Exception {
		...
		button_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!(canvas.getSelected() instanceof ChildFlowNode)) {
					MsgHelper.showMessage("请先选择一个类型为【子流程】的节点！");
					return;
				}
		...
		button_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (canvas.getSelected() == null) {
					MsgHelper.showMessage(null, "请先选择一个功能节点！");
					return;
				}

				if (!(canvas.getSelected() instanceof ProcessNode)) {
					MsgHelper.showMessage(null, "选择的节点不是一个功能节点，请重新选择后重试！");
					return;
				}

				Result result = ModelflowSelectDialog.showDialog(mainControl, canvas.getSelected().getId(), null);
				if (result != null) {
					DrawNode node = (DrawNode) canvas.getSelected();
					if (node != null) {
						if (canvas.getNode(result.id) != null) {
							MsgHelper.showMessage("选择的关联模块已经存在！");
							return;
						}
					}
	...
	public void save(boolean needPrompt) {
		try {
			...
			// if (MsgHelper.showConfirmDialog((this,
			// "配置信息文件已经存在，是否覆盖？",
