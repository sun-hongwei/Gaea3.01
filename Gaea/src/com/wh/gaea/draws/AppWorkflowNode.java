...
public abstract class AppWorkflowNode extends DrawNode {
	...
	protected void drawNode(Graphics g){
		...
		rect.height = height;
		IDrawNode.drawText(g, font, getFontColor(), rect, new StringBuilder(this.title == null ? "" : this.title));
		if (titleHeight > 0 && title != null && !title.isEmpty()){
			Rectangle titleRect = new Rectangle(rect.x, rect.y + height, rect.width, titleHeight);
			g.setColor(bgColor);
			IDrawNode.drawLine(g, titleRect.getLocation(), titleRect.width - 2);
			IDrawNode.drawText(g, font, getFontColor(), titleRect, new StringBuilder(title == null ? "" : title));
		}
	...
	public static abstract class CommandNode extends AppWorkflowNode{
		...
		public boolean createFile() throws Exception {
			...
			if (file.exists()){
				if (MsgHelper.showConfirmDialog(name + "节点设置的文件【" + file.getAbsolutePath() + "】已经存在，是否覆盖？", JOptionPane.YES_NO_OPTION) != JOptionPane.YES_OPTION)
					return false;
			}
