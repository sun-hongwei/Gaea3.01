...
public class ToolbarInfo extends DrawInfo {
	...
	public String typeName(){
		return IDrawInfoDefines.Toolbar_Name;
	}
	...
	public void reset(){
		...
		try {
			...
			for (int i = 0; i < datas.length(); i++) {
				IDrawNode childNode = IDrawNode.fromJson((DrawCanvas) node.getCanvas(), datas.getJSONObject(i), new ICreateNodeSerializable() {
					
					@Override
					public DrawNode newDrawNode(JSONObject json) {
						return new UINode((DrawCanvas) node.getCanvas());
					}
	...
	public void drawNode(Graphics g, Rectangle rect){	
		if (attachControlName != null && !attachControlName.isEmpty()){
			IUINode attachNode = null;
			for (IDrawNode node : node.getCanvas().getNodes()) {
	...
	public ToolbarInfo(IUINode node) {
		super(node);
		width = "400";
		height = "35";
		needFrame = false;
