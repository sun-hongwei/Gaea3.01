...
public class GridInfo extends DrawInfo{
		public String typeName(){
			return IDrawInfoDefines.Grid_Name;
		}

		public GridInfo(IUINode node) {
			super(node);
			width = "200px";
			height = "200px";
			value = "";
					...
					if (column.has("subs")){
						...
						for (int j = 0; j < subs.length(); j++) {
							...
							if (rect.contains(new Point(drawRect.x + drawRect.width, drawRect.y + drawRect.height))){
								...
								IDrawNode.drawLineText(g, getFont(), textColor, drawRect.x, drawRect.y, drawRect.width, drawRect.height, sub.getString("title"));
							}
							subLeft += subWidth;
							width += subWidth;
						}
...
//							DrawNode.drawLineText(g, getFont(), textColor, left, top, 80, realHeight, "");
						case "6":
							IDrawNode.drawLineText(g, getFont(), textColor, left, top, width, realHeight, "✔");
							break;
						default:
							IDrawNode.drawLineText(g, getFont(), textColor, left, top, width, realHeight, column.getString("title"));
							break;
						}
					}
					...
					for (int i = 0; i < values.length(); i++) {
						...
						while (keys.hasNext()){
							...
							if (rowCells.containsKey(key)){
								...
								IDrawNode.drawLineText(g, getFont(), textColor, r.x, r.y, r.width, r.height, v);
							}
						}
