...
import com.wh.gaea.draws.drawinfo.ReportInfo.CheckResult.ResultType;
...
public class ReportInfo extends DrawInfo {
	...
	protected void fireChange(IDrawNode node, ChangeType ct, Object data) {
	...
	public String typeName() {
		return IDrawInfoDefines.Report_Name;
	}

	public Template addTemplate(String name) {
		if (templates.containsKey(name)) {
			MsgHelper.showMessage("模板名称已经存在！");
			return null;
		}

		if (selects.size() == 0) {
			MsgHelper.showMessage("为选定任何单元格！");
			return null;
		}
	...
	public static class CellInfo {
		...
		public void fromJson(JSONObject json) throws Exception {
			...
			editor = (UINode) IDrawNode.fromJson((DrawCanvas) reportInfo.node.getCanvas(), json.getJSONObject("editor"),
					new ICreateNodeSerializable() {

						@Override
						public DrawNode newDrawNode(JSONObject json) {
							return new UINode((DrawCanvas) reportInfo.node.getCanvas());
						}
	...
	protected void newEditor(CellInfo info) {
		info.editor = new UINode((DrawCanvas) node.getCanvas());
			...
			if (count > rows.size()) {
				...
				for (CellInfo cellInfo : getCells().values()) {
					if (cellInfo.endRow > count) {
						needFire = false;
						MsgHelper.showMessage("单元格【" + cellInfo.editor.getDrawInfo().name + " - "
	...
	protected void drawNode(Graphics g, Rectangle rect, boolean clip) {
		...
		for (CellInfo info : getCells().values()) {
			...
			case IDrawInfoDefines.ComboTreeBox_Name: {
				...
				IDrawNode.drawLineText(g, info.editor.info.font, info.editor.info.textColor, r.x, r.y, r.width, r.height,
						text);
			}
				break;
			...
			case IDrawInfoDefines.TextArea_Name:
				IDrawNode.drawMulitText(g, info.editor.info.font, info.editor.info.textColor, r, valueText);
				break;
			...
			case IDrawInfoDefines.ListBox_Name:
				...
				try {
					...
					if (strValue != null && !strValue.isEmpty())
						IDrawNode.drawMulitText(g, info.editor.info.font, info.editor.info.textColor, r, strValue);
	...
	public ReportInfo(IUINode node) {
		super(node);
		width = "400";
		height = "400";
		needFrame = true;
