...
public class ListViewInfo extends DrawInfo {
	...
	public String typeName(){
		return IDrawInfoDefines.ListView_Name;
	}
	...
	protected ReportInfo getReportInfo(String name) {
		...
		IUINode reportNode = new UINode((DrawCanvas) node.getCanvas());
	...
	protected int drawHeader(Graphics g, Rectangle rect) throws JSONException {
		...
		for (Column column : columns.values()) {
			...
			IDrawNode.drawLineText(g, font, column.textcolor, r.x, r.y, r.width, r.height, column.title);
	...
	protected void drawItem(Graphics g, int top, JSONArray row) throws Exception {
		...
		for (int i = 0; i < row.length(); i++) {
			...
			case "text":
				...
				IDrawNode.drawLineText(g, font, color, r.x, r.y, r.width, r.height, text);
				break;
			case "image":
				if (!data.has("image"))
					continue;
	...
	public ListViewInfo(IUINode node) {
		super(node);
		width = "400";
		height = "400";
		needFrame = true;
