...
public class TreeInfo extends DrawInfo{
	public String typeName(){
		return IDrawInfoDefines.Tree_Name;
	}
	public TreeInfo(IUINode node) {
		super(node);
		width = "30%";
		height = "35%";
	...
	protected void drawItem(Rectangle rect, Graphics g, int left, AtomicInteger top, int width, String text){
		...
		int height = IDrawNode.drawLineText(g, getFont(), textColor, drawTextLeft, top.get(), drawTextWidth, -1, text, false, true);
		if (!rect.contains(new Point(left, top.get() + height)))
			return;

		g.drawImage(icon, left, top.get(), null);
		
		IDrawNode.drawLineText(g, getFont(), textColor, drawTextLeft, top.get(), drawTextWidth, -1, text, false);
