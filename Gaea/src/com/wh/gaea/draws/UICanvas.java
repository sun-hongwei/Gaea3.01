...
import com.wh.gaea.draws.drawinfo.ControlInfo;
...
public class UICanvas extends DrawCanvas {
	...
	protected JSONArray getNavTreeInfo(IDrawNode node) {
	...
	protected void saveNavTreeInfo(IDrawNode node) throws Exception {
	...
	public void setMainNavTree(String uiid, String id) throws Exception {
		EditorEnvironment.traverseUI(new IEditorInterface.ITraverseUIFile() {

			@Override
			public boolean callback(File uiFile, IDrawCanvas drawCanvas, Object userObject) {
				UICanvas canvas = (UICanvas) drawCanvas;
	...
	protected void paintNodes(Graphics g, Collection<IDrawNode> nodes, boolean needCheckViewport) {
		...
		for (IDrawNode node : controlInfos) {
	...
	public IUINode add(String title, String typename, int width, int height) {
	...
	public UINode add(String name, String typename, Rectangle r) {
		return (UINode) super.add(name, null, null, null, new IDataSerializable() {
			...
			public void initDrawNode(IDrawNode node) {
				...
				DrawInfo info = UINode.newInstance(typename, (IUINode) node);
				Point pt = node.getCanvas()
						.getRealPoint(new Point(canvas.getWidth() / 2, canvas.getHeight() / 2));
