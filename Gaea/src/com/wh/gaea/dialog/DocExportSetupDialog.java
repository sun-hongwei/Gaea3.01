...
import com.wh.tools.WordDocument.PageNumberHeaderInfo;;
...
public class DocExportSetupDialog extends JDialog {
	...
	protected void deleteItem(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_DELETE){
			if (MsgHelper.showConfirmDialog("是否删除项目【" + list.getSelectedValue() + "】", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION){
				...
				panel_1.setLayout(new FlowLayout(FlowLayout.CENTER, 5, 5));
				{
					...
					});
					{
						...
						button_1.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								File file = SwingTools.selectOpenImageFile(null, null, "");
					...
					buttonPane.add(panel, BorderLayout.EAST);
					{
						...
						button.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								...
								if (!isFile){
									File file = SwingTools.selectSaveDir(null, "选择导出路径", "请选择你要保存的目录位置", defaultFile.getAbsolutePath());
									if (file == null)
										return;
								...
								File file = SwingTools.selectSaveFile(null, defaultFile.getAbsolutePath(), EditorEnvironment.getCurrentProjectName() + "系统详细设计", "Word文档（2007及以后版本）=docx");
								if (file == null)
									return;
								
								if (file.exists()){
									if (MsgHelper.showConfirmDialog("文件：" + file.getAbsolutePath() + "已经存在，是否覆盖？", JOptionPane.YES_NO_OPTION) != JOptionPane.YES_OPTION)
										return;
	...
	public static Result showDialog(String[] headers, String[] footers, File saveFile, boolean isFile){
		...
		if (!path.exists())
			if (!path.mkdirs()){
				MsgHelper.showException(new Exception("无法创建目录：" + path.getAbsolutePath()));
				return null;
			}
