package com.wh.gaea.control.grid.design;
...
import com.wh.gaea.draw.DrawInfo.PlaceAlign;
import com.wh.gaea.draw.DrawInfo.PlaceInType;
import com.wh.gaea.draw.DrawInfo.ScaleMode;
...
import com.wh.gaea.draws.drawinfo.ChartInfo.ChartType;
...
import com.wh.gaea.draws.drawinfo.LabelInfo.TextAlign;
import com.wh.gaea.draws.drawinfo.LabelInfo.TextWay;
...
import com.wh.gaea.editor.JsonTreeDataConfigDialog;
...
import com.wh.gaea.interfaces.IClientEvent;
...
import com.wh.gaea.interfaces.input.ListInput;
import com.wh.gaea.role.RoleInfos;
import com.wh.gaea.selector.CheckSelector;
...
import com.wh.swing.tools.combobox.MulitCombobox;
import com.wh.swing.tools.dialog.input.MulitTextInput;
...
public class DefaultPropertyClient implements IClientEvent {
	...
	public static class PropertyInfo implements IProperty {
		...
		public PropertyInfo(Object node, String name, String title, Object defaultValue, DrawNode workflowNode,
				HashMap<String, IDrawNode> workflowNodes, String workflowRelationTitle, List<DataSource> dataSources) {
			if (dataSources != null)
		...
		public JComponent getEditor() {
			...
			case "textWay":
				if (node instanceof UINode) {
					JComboBox<TextWay> combo = new JComboBox<>(TextWay.values());
					editor = combo;
					combo.setEditable(false);
					String vString = getValue();
					if (vString != null && !vString.isEmpty()){
						TextWay textWay = TextWay.valueOf(vString);
						combo.setSelectedItem(textWay);
					}
				}
				break;
			case "textAlign":
				if (node instanceof UINode) {
					JComboBox<TextAlign> combo = new JComboBox<>(TextAlign.values());
					editor = combo;
					combo.setEditable(false);
					String vString = getValue();
					if (vString != null && !vString.isEmpty()){
						TextAlign textAlign = TextAlign.valueOf(vString);
						combo.setSelectedItem(textAlign);
					}
				}
				break;
			case "divType":
			...
			case "value":
				...
				switch (((IUINode) node).getDrawInfo().typeName()) {
			...
			case "timer_type":
				...
				break;
			case "mode":
				if (((IUINode) node).getDrawInfo() instanceof ComboInfo)
			...
			if (editor != null) {
				...
				String typename = "";
				if (node instanceof UINode)
					typename = ((IUINode) node).getDrawInfo().typeName();
	...
	public static void propertyBuilder(PropertyPanel table, UINode node, DrawNode workflowNode,
			HashMap<String, IDrawNode> workflowNodes, String workflowRelationTitle) throws Exception {
	...
	public boolean checkControlIDRepeat(String id, IUINode idnode) {
		for (IDrawNode tmp : canvas.nodes.values()) {
			IUINode node = (IUINode) tmp;
	...
	public boolean checkControlNameRepeat(String name, IUINode idnode) {
		for (IDrawNode tmp : canvas.nodes.values()) {
			IUINode node = (IUINode) tmp;
	...
	public String selectImage(Object sender) {
		File imageFile = SwingTools.selectOpenImageFile(iUpdate.getParent(), null, null);
		if (imageFile == null)
			return null;
		...
		try {
			...
			e.printStackTrace();
			MsgHelper.showMessage(iUpdate.getParent(), "拷贝文件失败！", "设置失败", JOptionPane.ERROR_MESSAGE);
			return null;
		}
	}
	...
	protected boolean updateValue(JComponent sender, String name, Object value) {
		...
		try {
			...
			Class<?> c = null;
			if (node instanceof UINode)
				object = ((IUINode) node).getDrawInfo();
			...
			if ((isID || isName) && node instanceof UINode) {
				...
				if (value == null || v.isEmpty()) {
					MsgHelper.showMessage(iUpdate.getParent(), (isName ? "name" : "id") + "不能为空，请重新输入！", "提示");
					return false;
				}

				if (!checkNameFormat(v)) {
					MsgHelper.showMessage(iUpdate.getParent(), "输入的字符格式不符合要求，请重新输入！", "提示");
					return false;
				}

				if (isID) {
					if (checkControlIDRepeat(v, (IUINode) node)) {
						MsgHelper.showMessage(iUpdate.getParent(), "id已经存在，请重新输入！", "提示");
						return false;
					}
				}

				if (isName) {
					if (checkControlNameRepeat(v, (IUINode) node)) {
						MsgHelper.showMessage(iUpdate.getParent(), "name已经存在，请重新输入！", "提示");
						return false;
					}
				}
			}
			...
			if (object instanceof ReportInfo) {
				ReportInfo reportInfo = (ReportInfo)object;
			...
			e.printStackTrace();
			MsgHelper.showMessage(iUpdate.getParent(), "不能设置属性值：" + SwingTools.getExceptionMsg(e) + "！", "设置失败",
	...
	public void onClick(JComponent sender, int row, int col, ISetValue onSetValue) {
		...
		case "attachControlName": {
			ButtonLabel ob = (ButtonLabel) sender;
			ControlSelectDialog.Result result = ControlSelectDialog.showDialog(iUpdate.getUIID(),
					null, true);
			if (result.isok) {
				if (result.data == null)
					value = null;
				else
					value = ((IUINode) result.data[0]).getDrawInfo().name;
				ob.setValue(value);
				break;
			} else
				return;
		...
		case "decideValue": {
			ButtonLabel ob = (ButtonLabel) sender;
			String result = GlobalInstance.instance().getWorkflowSelector().selectDecideValue((String) ob.getValue());
			value = result;
			ob.setValue(value);
			break;
		}
		...
		case "fireDataSource": {
			...
			com.wh.gaea.editor.ControlSelectDialog.Result result = ControlSelectDialog.showDialog(uiid, ids, null,
					true, true);
			if (result.isok) {
				sets = new JSONArray();
				if (result.data != null)
					for (IDrawNode drawNode : result.data) {
				...
				public boolean onCheck(Object[][] originalData, JTable table) {
					...
					for (int i = 0; i < table.getRowCount(); i++) {
						...
						if (dsid == null || dsid.isEmpty()) {
							MsgHelper.showMessage("第【" + ++i + "】行的数据源不能为null！");
							return false;
						}

						if (param == null || param.isEmpty()) {
							MsgHelper.showMessage("第【" + ++i + "】行的参数不能为null！");
							return false;
						}

						String key = dsid + param;
						if (map.containsKey(key)) {
							MsgHelper.showMessage("第【" + ++i + "】行的参数已经在第【" + map.get(key) + "】行定义！");
							return false;
						}
						...
						case 1:
							...
							case SQLDataSource.SQL_KEY:
								if (ds.params.has("sql")) {
									String sql = ds.params.getString("sql");
									Map<String, String> params = GlobalInstance.instance().getDataSourceSelector().parseSQL(sql);
									selects.addAll(params.values());
								}
								break;
							}
							break;
		...
		case "role": {
			...
			if (ob.getValue() instanceof JSONArray) {
				value = new RoleInfos((JSONArray) ob.getValue());
			}
			RoleInfos result = GlobalInstance.instance().getRoleSelector().selectRoles((RoleInfos) value);
			if (result != null) {
				value = result;
				ob.setValue(result.toJson());
			}
		...
		case "data":
			...
			case IDrawInfoDefines.ComboTreeBox_Name: {
				...
				try {
					...
					e.printStackTrace();
					MsgHelper.showMessage(iUpdate.getParent(), "json数据格式错误！", "设置失败",
							JOptionPane.ERROR_MESSAGE);
					return;
				}
				ob.setValue(value);
				IUINode node = (IUINode) sender.getClientProperty("node");
				...
				if (model != null) {
					...
					for (int i = 0; i < model.getRowCount(); i++) {
						...
						try {
							...
							e.printStackTrace();
							MsgHelper.showMessage(iUpdate.getParent(), "设置value属性失败，请检查您的输入 ！", "设置失败",
									JOptionPane.ERROR_MESSAGE);
							return;
						}
						datas.put(rowData);
		...
		case "extendData": {
			...
			try {
				...
				e.printStackTrace();
				MsgHelper.showMessage(iUpdate.getParent(), "json数据格式错误！", "设置失败", JOptionPane.ERROR_MESSAGE);
				return;
			}
			break;
		}
		case "closeDialogId": {
			ButtonLabel ob = (ButtonLabel) sender;
			com.wh.gaea.editor.ControlSelectDialog.Result result = ControlSelectDialog.showDialog(iUpdate.getUIID(),
			...
			if (result.isok) {
				...
				IUINode node = (IUINode) result.data[0];
