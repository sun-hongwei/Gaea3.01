...
public class ControlTreeManager {
	...
	public static class UITreeCellRender extends DefaultTreeCellRenderer {
		...
		public Component getTreeCellRendererComponent(JTree tree, Object value, boolean sel, boolean expanded,
			...
			if (!(treeNode.getUserObject() instanceof UINode)) {
				...
				try {
					IUINode node = (IUINode) treeNode.getUserObject();
	...
	public static class WorkflowTreeCellRender extends DefaultTreeCellRenderer {
		...
		public WorkflowTreeCellRender(IDrawCanvas canvas) {
			this.canvas = canvas;
		}
	...
	public ControlTreeManager(JTree tree, IDrawCanvas canvas, String rootName) {
	...
	public void selectTreeNode(IDrawNode node) {
		if (node == null) {
		...
		for (int i = 0; i < root.getChildCount(); i++) {
			...
			if (treeNode.getUserObject() instanceof UINode) {
				...
				if (info.id.compareTo(node.getId()) == 0) {
				...
				if (node.getName().compareToIgnoreCase(n.name) == 0) {
					selectedTreeNode(treeNode);
					return;
				}
			}
	...
	HashMap<IDrawNode, DefaultMutableTreeNode> treeNodes = new HashMap<>();

	public void addToTree(IDrawNode info) {
		DefaultTreeModel model = (DefaultTreeModel) tree.getModel();
	...
	public void refreshTree() {
		...
		if (canvas.getSelected() != null)
			selectedTreeNode((DefaultMutableTreeNode) model.getRoot(), canvas.getSelected().getId());
	...
	public IDrawNode selectCanvasNode(TreeNode treenode) {
		if (treenode == null)
			return null;
		...
		if (!(treeNode.getUserObject() instanceof DrawNode))
			return null;
		IDrawNode node = (IDrawNode) treeNode.getUserObject();
		if (node != null) {
